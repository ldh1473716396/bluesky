...
class Ready extends Index 
{
    ...
    public function addready($event_id)
    {
    	...
    	if ($judge) {
    		$this->error('您已经填写过，无需再填写。');
    	}

        $ready_conf = \app\admin\model\ReadyConf::order('ready_conf_sort','asc')->select();
        ...
        $this->assign(['conf'=>$ready_conf,'event'=>$event]);
        return view();
    }
    ...
    public function editready($id,$event_id)
    {
        ...
        $this->assign(['conf'=>$ready_conf,'edit'=>$edit,'event'=>$event]);
        return view();
    }
