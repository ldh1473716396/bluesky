...
<div style="text-align: center; margin-top: 55px; margin-bottom: 50px;">
	<h1>{$sheet.sheet_name}填写</h1>
	<h4>事件：{$event.event_title}</h4>
	<h5><span style="color: #FF5722;">（禁止填入“ | ”，“ * ”，“ - ” 等符号）</span><h5>
</div>
		
<div class="container" style="margin-bottom: 100px;">
<form action="{:url('team/team_record/add',array('event_id'=>$event.id,'sheet_id'=>$sheet.id))}" enctype="multipart/form-data" method="post">
...
<label>{$data.sheet_conf_name}：</label>
	...
	<input type="text" class="form-control" name="team_content[{$data.id}]" required>
	{/if}

	<!--2文本框-->
	{if $data.sheet_conf_type == 2}
	<textarea type="text" class="form-control" name="team_content[{$data.id}]" rows="3" required></textarea>
	{/if}
	...
	<?php foreach($data['sheet_conf_values'] as $k3 => $v3): ?>
		...
		<input class="form-check-input" type="radio" id="{$data.id}[{$v3}]" name="team_content[{$data.id}]"  value="{$v3}" required>
	...
	<?php foreach($data['sheet_conf_values'] as $k4 => $v4): ?>
		<div class="form-check form-check-inline">
		<input class="form-check-input" type="checkbox" id="{$v4}" name="team_content[{$data.id}][{$v4}]" value="{$v4}">
