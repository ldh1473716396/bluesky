...
<form action="{:url('team/ready/add',array('event_id'=>$event.id))}" enctype="multipart/form-data" method="post">
...
<label>{$data.ready_conf_key}：</label>
	...
	<input type="text" class="form-control" name="ready_content[{$data.id}]" required>
	{/if}

	<!--2文本框-->
	{if $data.ready_conf_type == 2}
	<textarea type="text" class="form-control" name="ready_content[{$data.id}]" rows="3" required></textarea>
	{/if}
	...
	<?php foreach($data['ready_conf_value'] as $k3 => $v3): ?>
		...
		<input class="form-check-input" type="radio" id="{$data.id}[{$v3}]" name="ready_content[{$data.id}]"  value="{$v3}" required>
