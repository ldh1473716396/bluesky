...
class Event extends Index 
{
    ...
    {
        $list = \app\admin\model\Event::order('update_time','desc')->field('id,event_title,event_state,update_time')->select();

        if ($list) {
            return json_encode($list);
        }
        else{
            $arr=['message'=>'暂无数据','code'=>400];
    ...
    public function readevent($id)
    {
        ...
        $event_conf = \app\admin\model\EventConf::order('id','asc')->select();      
        ...
        //备勤名单
        $ready_conf = \app\admin\model\ReadyConf::field('id,ready_conf_key')->select();
        ...
        $start_conf = $modelStartConf->field('id,start_conf_key')->select();
        ...
        if (isset($ready_team_id)) {
            ...
            in_array(session('team_id'), $ready_team_id)?$log_limit=1:$log_limit=0;           
        }
        else{
            $log_limit=0;
        }
        $log_conf = \app\admin\model\LogConf::field('id,log_conf_key')->select();
        ...
        //$this->assign(['log_limit'=>$log_limit,'log_conf'=>$log_conf,'log'=>$log]);

        return json_encode([
        	'read'=>$read,'event_conf'=>$event_conf,
        	'admin_sheet'=>$admin_sheet,
        	'ready'=>$ready,'ready_conf'=>$ready_conf,
        	'start'=>$start,'start_conf'=>$start_conf,
        	'log'=>$log,'log_conf'=>$log_conf,'log_limit'=>$log_limit,
